...
import Newenter from '../newEnter/Newenter'
import Entrantmodif from '../entrantmodif/Entrantmodif'
import './entrant.css'
import Fade from '../fade/Fade'
import Suppenter from '../suppenter/Suppenter'
...
function Entrant() {
    ...
    const handeleSearchUsingLike = (e)=>{
        axios
        .post("http://localhost/api/searchEnterUsingLike",{like: e.target.value ,ideglise: user.ideglise})
        .then(res=>{
            setEnterData(res.data)
            console.log(res)
        })
    ...
    const deleteIt = (idcle)=>{
        axios
        .patch("http://localhost/api/Suppentre",{idcle})
        .then(res=>{
            console.log(res)
    ...
    <>
        <div className="entrant">
            <h2>Mouvement <span>Entrant en Caisse</span></h2>
            ...
            <div className="content__bottom">
                <div className="heading">
                    <span className="heading__title">Table Entré :</span>
                    ...
                    <table>
                        ...
                        <tbody>
                          ...
                          return  <tr key={index}>
                                    ...
                                    <td><button className="btn success" onClick={()=>Modification(data)}>Modifier</button> <button className="btn danger" onClick={()=>showMess(data.idcle,data.montantEntre)}>Supprimer</button></td>
                                </tr>
                        })}
                        </tbody>
                    </table>
        ...
        {add?<Newenter setadd={setadd} enterData={enterData} setEnterData={setEnterData}/>:null}
        {modif? <Entrantmodif setmodif={setmodif} modifData={modifData} setEnterData={setEnterData}/> : null}
        {custumMess && <Fade>
               <Suppenter setCustumMess={setCustumMess} deleteIt={deleteIt} suppData={suppData}/>
        </Fade>}
    </>
    )
}
